...
#ifdef DHCPLEASED_PRIV_ONLY
#define IMSG_INVALID	(IMSG_REQUEST_REBOOT + 1)
...
main(int argc, char *argv[])
{
...
#endif /* SMALL */
#ifdef DHCPLEASED_PRIV_ONLY
	struct imsgbuf		 frontend_fuzz_ibuf, engine_fuzz_ibuf;
	...
	/* parse config file */
#ifdef DHCPLEASED_PRIV_ONLY
	if ((main_conf = calloc(1, sizeof(*main_conf))) == NULL)
		fatal(NULL);
	SIMPLEQ_INIT(&main_conf->iface_list);
#else
...
#ifdef DHCPLEASED_PRIV_ONLY
	/* Install the synthetic child endpoints before parent setup messages. */
	if (imsgbuf_init(&frontend_fuzz_ibuf, pipe_main2frontend[1]) == -1 ||
	    imsgbuf_init(&engine_fuzz_ibuf, pipe_main2engine[1]) == -1)
		fatal(NULL);
#endif
...
#else
	/*
	 * Privileged-compartment harness: disable host-mutating privileged
	 * resources while keeping the original parent setup messages below.
	 * The original parent handlers still parse and act on generated imsg
	 * payloads; route/ioctl/lease side effects fail closed.
	 */
	(void)saved_argv0;
	routesock = -1;
	ioctl_sock = -1;
...
#else
	/*
	 * Preserve every privileged-to-unprivileged setup message.  The IPC
	 * socket pair is safe to create unchanged.  A real route socket is
	 * passed to the synthetic frontend, but host-specific route filtering
	 * and other host-mutating setup remain disabled in this fuzz build.
	 */
	if (main_imsg_send_ipc_sockets(&iev_frontend->ibuf,
	    &iev_engine->ibuf))
		fatal("could not establish imsg links");
	if ((frontend_routesock = socket(AF_ROUTE,
	    SOCK_RAW | SOCK_CLOEXEC, AF_INET)) == -1)
		fatal("route socket");
	main_imsg_compose_frontend(IMSG_ROUTESOCK, frontend_routesock, NULL, 0);
	if (socketpair(AF_UNIX, SOCK_STREAM | SOCK_CLOEXEC,
	    PF_UNSPEC, control_pair) == -1)
		fatal("control socketpair");
	...
	/*
	 * Two modeled senders: endpoint 0 is the unprivileged frontend, and
	 * endpoint 1 is the unprivileged engine.  Map the complete production
	 * enum plus one invalid value; the parent dispatchers decide which
	 * messages each sender may use.
	 */
	if (msg_interface_init(&fuzz_iface, STDIN_FILENO, 2,
	    IMSG_INVALID - IMSG_NONE + 1) !=
	    MSG_GEN_SUCCESS)
		fatalx("msg_interface_init failed");
	msg_interface_set_endpoint(&fuzz_iface, 0, &frontend_fuzz_ibuf);
	msg_interface_set_endpoint(&fuzz_iface, 1, &engine_fuzz_ibuf);
	...
	while (msg_generate(&fuzz_iface, &fuzz_msg) == MSG_GEN_SUCCESS) {
		...
		/*
		 * dhcpleased uses fd passing for parent-to-child setup, but the
		 * privileged parent handlers fuzzed here do not accept fds from
		 * the children.  Close any fd the generic generator created and
		 * keep injected child-to-parent messages on the original no-fd
		 * request paths.
		 */
		if (fuzz_msg.fd != -1) {
			close(fuzz_msg.fd);
		...
		/*
		 * The payload bytes come from the seed, not from the privileged
		 * compartment.  Mark them as expected input before composing the
		 * imsg so pointer-like seed values do not mask real leaks from
		 * buffers produced by the parent code under test.
		 */
	...
	/*
	 * Stdin is exhausted.  Send one EOM to each synthetic child endpoint so
	 * the event loop drains pending messages and exits through the shared
	 * eom_counter path in the original dispatchers.
	 */
