...
#ifdef OSPFD_PRIV_ONLY
static int ospfd_fuzz_main(void);
#endif
...
#ifdef OSPFD_PRIV_ONLY
static int
ospfd_fuzz_main(void)
{
	struct msg_interface iface;
	struct msg_data msg, eom;
	struct imsgbuf ospfe_fuzz_ibuf, rde_fuzz_ibuf;
	int control_pair[2];

	ospfd_process = PROC_MAIN;
	ospfd_conf = calloc(1, sizeof(*ospfd_conf));
	if (ospfd_conf == NULL)
		fatal(NULL);
	LIST_INIT(&ospfd_conf->area_list);
	LIST_INIT(&ospfd_conf->cand_list);
	SIMPLEQ_INIT(&ospfd_conf->redist_list);
	log_init(1, LOG_DAEMON);
	event_init();
	if (socketpair(AF_UNIX, SOCK_STREAM | SOCK_CLOEXEC | SOCK_NONBLOCK,
	    PF_UNSPEC, pipe_parent2ospfe) == -1 ||
	    socketpair(AF_UNIX, SOCK_STREAM | SOCK_CLOEXEC | SOCK_NONBLOCK,
	    PF_UNSPEC, pipe_parent2rde) == -1)
		fatal("socketpair");
	iev_ospfe = calloc(1, sizeof(*iev_ospfe));
	iev_rde = calloc(1, sizeof(*iev_rde));
	if (iev_ospfe == NULL || iev_rde == NULL)
		fatal(NULL);
	if (imsgbuf_init(&iev_ospfe->ibuf, pipe_parent2ospfe[0]) == -1 ||
	    imsgbuf_init(&iev_rde->ibuf, pipe_parent2rde[0]) == -1 ||
	    imsgbuf_init(&ospfe_fuzz_ibuf, pipe_parent2ospfe[1]) == -1 ||
	...
	/* Preserve the parent's original privileged setup descriptor send. */
	if (socketpair(AF_UNIX, SOCK_STREAM | SOCK_CLOEXEC,
	    PF_UNSPEC, control_pair) == -1)
		fatal("control socketpair");
	main_imsg_compose_ospfe_fd(IMSG_CONTROLFD, 0, control_pair[0]);

	if (msg_interface_init(&iface, STDIN_FILENO, 2,
	    IMSG_IFADDRDEL - IMSG_NONE + 2) !=
	    MSG_GEN_SUCCESS)
		fatalx("msg_interface_init");
	msg_interface_set_endpoint(&iface, 0, &ospfe_fuzz_ibuf);
	msg_interface_set_endpoint(&iface, 1, &rde_fuzz_ibuf);
	msg_interface_set_eom_type(&iface, IMSG_EOM);
	if (msg_interface_set_message_types_batch(&iface, 0,
	    IMSG_IFADDRDEL - IMSG_NONE + 2, IMSG_NONE) != MSG_GEN_SUCCESS)
		fatalx("setting message types failed");
	while (msg_generate(&iface, &msg) == MSG_GEN_SUCCESS) {
		struct imsgbuf *target = msg.endpoint;
		if (msg.fd != -1) {
			close(msg.fd);
			msg.fd = -1;
		}
		...
		imsg_compose(target, msg.type, 0, 0, -1, msg.payload,
		    msg.actual_payload_size);
		imsgbuf_flush(target);
	}
	for (i = 0; i < iface.num_compartments; i++) {
		if (msg_generate_eom(&iface, (uint8_t)i, &eom) !=
		    MSG_GEN_SUCCESS || eom.endpoint == NULL)
			continue;
		imsg_compose(eom.endpoint, eom.type, 0, 0, -1, NULL, 0);
		imsgbuf_flush(eom.endpoint);
	}
	event_dispatch();
