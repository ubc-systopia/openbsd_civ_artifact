# eigrpd privileged-compartment harness

Copy the `ptr_checker` directory next to this README before building.

This directory fuzzes the original privileged `eigrpd` parent dispatcher.  The
harness is embedded under `EIGRPD_PRIV_ONLY`, and vendored imsg is dynamically
linked so outbound compartment messages are checked at the compose boundary.
...
Build and fuzz the ASan/UBSan plus pointer-checking configuration on FreeBSD:
...
afl-fuzz -i seeds -o findings-asan -- ./eigrpd-priv-harness
```
...
afl-fuzz -i seeds -o findings-msan -- ./eigrpd-priv-harness
```
