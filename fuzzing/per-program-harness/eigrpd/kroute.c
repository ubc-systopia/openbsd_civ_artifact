...
#ifdef EIGRPD_PRIV_ONLY
	/*
	 * The harness fuzzes child-to-parent messages, not kernel routing
	 * socket events.  Keep the in-memory route state available for the
	 * original parent handlers, but do not attach a real route socket or
	 * parse the host routing table.
	 */
	kr_state.fd = -1;
	kr_state.pid = getpid();
	kr_state.rtseq = 1;
