    ...
    if (msg->has_fd) {
        ...
        if (msg->actual_fd_size > 0) {
            ...
            while (total_written < msg->actual_fd_size) {
                ptr_check_skip(msg->fd_payload + total_written,
...
int
msg_sequence_plan(const struct msg_data *msg, struct msg_sequence *seq)
{
    uint16_t raw_length, remaining;
    uint8_t i;

    if (msg == NULL || seq == NULL) {
        return MSG_GEN_ERROR;
    }

    memset(seq, 0, sizeof(*seq));
    seq->count = 1 + (uint8_t)msg->aux_data[0] %
        MAX_MESSAGES_PER_TYPE;
    seq->fd_message = msg->has_fd ?
        (uint8_t)msg->aux_data[1] % seq->count : MSG_SEQUENCE_NO_FD;

    remaining = msg->actual_payload_size;
    for (i = 0; i < seq->count; i++) {
        seq->offset[i] = msg->actual_payload_size - remaining;
        if (i == seq->count - 1) {
            seq->length[i] = remaining;
        } else {
            memcpy(&raw_length, &msg->aux_data[2 + i * sizeof(uint16_t)],
                sizeof(raw_length));
            seq->length[i] = remaining == UINT16_MAX ? raw_length :
                raw_length % (remaining + 1);
        }
        remaining -= seq->length[i];
    }
