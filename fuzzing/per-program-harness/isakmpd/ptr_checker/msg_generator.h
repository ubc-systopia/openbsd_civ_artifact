...
/* Bounded sub-message planning within one generated message type. */
#define MAX_MESSAGES_PER_TYPE 16
#define MSG_SEQUENCE_NO_FD UINT8_MAX
...
/*
 * A generated message can be exposed to a harness as a bounded sequence of
 * payload slices.  The slices preserve the original payload byte-for-byte;
 * they only describe write boundaries and the point at which an optional FD
 * should be transferred.
 */
struct msg_sequence {
    uint8_t count;
    uint8_t fd_message;
    uint16_t offset[MAX_MESSAGES_PER_TYPE];
    uint16_t length[MAX_MESSAGES_PER_TYPE];
};
...
/**
 * Derive a bounded sub-message plan from msg->aux_data.
 *
 * aux_data[0] controls the number of slices (1..MAX_MESSAGES_PER_TYPE),
 * aux_data[1] selects the slice that carries msg->fd, and aux_data[2..33]
 * contains sixteen native-endian uint16_t requested slice lengths.  Lengths
 * are clamped to the generated payload; the final slice consumes all bytes
 * that remain.
 */
int msg_sequence_plan(const struct msg_data *msg, struct msg_sequence *seq);
