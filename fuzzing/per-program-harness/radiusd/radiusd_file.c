...
main(int argc, char *argv[])
{
...
#ifdef RADIUSD_FILE_PRIV_ONLY
	pid = 0;
	if (imsgbuf_init(&sender, pairsock[1]) == -1)
		err(EXIT_FAILURE, "imsgbuf_init sender");
	if (msg_interface_init(&iface, STDIN_FILENO, 1,
	    IMSG_RADIUSD_FILE_USERINFO - IMSG_RADIUSD_FILE_OK + 2) !=
	    MSG_GEN_SUCCESS)
		errx(EXIT_FAILURE, "msg_interface_init");
	msg_interface_set_endpoint(&iface, 0, &sender);
	if (msg_interface_set_message_types_batch(&iface, 0,
	    IMSG_RADIUSD_FILE_USERINFO - IMSG_RADIUSD_FILE_OK + 2,
	    IMSG_RADIUSD_FILE_OK) != MSG_GEN_SUCCESS)
		errx(EXIT_FAILURE, "set message type range");
	msg_interface_set_eom_type(&iface, IMSG_RADIUSD_FILE_EOM);
	...
	if (imsg_compose(&sender, msg.type, 0, 0, -1, msg.payload,
	    msg.actual_payload_size) == -1 || imsgbuf_flush(&sender) == -1)
		err(EXIT_FAILURE, "send startup message");
#else
...
#ifdef RADIUSD_FILE_PRIV_ONLY
	/* Queue fuzz messages after the synchronous startup exchange. */
	while (msg_generate(&iface, &msg) == MSG_GEN_SUCCESS) {
		...
		if (imsg_compose(&sender, msg.type, 0, 0, -1, msg.payload,
		    ...
		    imsgbuf_flush(&sender) == -1)
			err(EXIT_FAILURE, "send fuzz message");
	}
	if (msg_generate_eom(&iface, 0, &eom) == MSG_GEN_SUCCESS) {
		if (imsg_compose(&sender, eom.type, 0, 0, -1, NULL, 0) == -1 ||
		    imsgbuf_flush(&sender) == -1)
			err(EXIT_FAILURE, "send EOM");
	}
#endif
