# npppd privileged-compartment harness

Copy the `ptr_checker` directory next to this README before building.

This directory fuzzes `npppd`'s original privileged `privsep` dispatcher.  The
guarded `NPPPD_PRIV_ONLY` path supplies generated imsgs through the native
protocol, while dynamically linked vendored imsg exposes outbound compose calls
to the checker.
The generator covers the complete `enum imsg_code` range and one value beyond
that range; the privileged dispatcher decides which values are requests.
...
afl-fuzz -i seeds -o findings-asan -- ./npppd-priv-harness
```
...
afl-fuzz -i seeds -o findings-msan -- ./npppd-priv-harness
```
