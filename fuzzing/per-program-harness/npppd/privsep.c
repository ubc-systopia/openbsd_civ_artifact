...
#ifdef NPPPD_PRIV_ONLY
int
npppd_priv_fuzz_main(void)
{
	struct msg_interface iface;
	struct msg_data msg;
	struct imsgbuf fuzz_ibuf, priv_ibuf;
	int pair[2], flags;
	...
	if (flags == -1 || fcntl(pair[1], F_SETFL, flags | O_NONBLOCK) == -1)
		return (1);
	if (imsgbuf_init(&priv_ibuf, pair[0]) == -1 ||
	    imsgbuf_init(&fuzz_ibuf, pair[1]) == -1)
		return (1);
	imsgbuf_allow_fdpass(&priv_ibuf);
	imsgbuf_allow_fdpass(&fuzz_ibuf);
	if (msg_interface_init(&iface, STDIN_FILENO, 1,
	    PRIVSEP_SET_IF_FLAGS - PRIVSEP_OK + 2) != MSG_GEN_SUCCESS)
		return (1);
	msg_interface_set_endpoint(&iface, 0, &fuzz_ibuf);
	if (msg_interface_set_message_types_batch(&iface, 0,
	    PRIVSEP_SET_IF_FLAGS - PRIVSEP_OK + 2, PRIVSEP_OK) !=
	    MSG_GEN_SUCCESS)
		return (1);

	if (msg_generate(&iface, &msg) == MSG_GEN_SUCCESS) {
		ptr_check_skip(msg.payload, msg.actual_payload_size);
		if (imsg_compose(&fuzz_ibuf, msg.type, 0, 0, msg.fd,
		    msg.payload, msg.actual_payload_size) == -1)
			goto done;
		msg.fd = -1;
		if (imsgbuf_flush(&fuzz_ibuf) == -1)
			goto done;
	}
done:
	signal(SIGPIPE, SIG_IGN);
	imsgbuf_clear(&fuzz_ibuf);
	close(pair[1]);
	privsep_priv_dispatch_imsg(&priv_ibuf);
	imsgbuf_clear(&priv_ibuf);
	close(pair[0]);
	return (0);
}
#endif
