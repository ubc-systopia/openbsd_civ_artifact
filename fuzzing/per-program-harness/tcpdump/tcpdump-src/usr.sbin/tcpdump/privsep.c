...
priv_init(int argc, char **argv)
{
	...
	/*
	 * Harness: do not fork.  Re-exec self with -P so the privileged
	 * branch (priv_exec) runs in this single process and drives the
	 * fuzzer-controlled message stream.
	 */
	(void)socks;
...
priv_exec(int argc, char *argv[])
{
...

	{
		...
		if (msg_interface_init(&iface, STDIN_FILENO, 1, 13) != MSG_GEN_SUCCESS)
			errx(1, "Failed to initialize message interface");

		msg_interface_set_endpoint(&iface, 0, &socks[1]);
		msg_interface_set_eom_type(&iface, PRIV_EOM);

		struct msg_data msg;
		while (msg_generate(&iface, &msg) == MSG_GEN_SUCCESS) {
			int *dest_socket = (int *)msg.endpoint;
			if (dest_socket == NULL)
				continue;
			ptr_check_skip(msg.payload, msg.actual_payload_size);
			must_write(*dest_socket, &msg.type, sizeof(int));
			must_write(*dest_socket, msg.payload, msg.actual_payload_size);
			if (msg.has_fd)
				send_fd(*dest_socket, msg.fd);
		}

		/*
		 * Send PRIV_EOM so the receiver's switch dispatches into
		 * eom_counter_inc() and exit(0)s cleanly when fuzzer input
		 * is exhausted.  Without this, may_read() blocks and AFL
		 * times every test case out.
		 */
		if (msg_generate_eom(&iface, 0, &msg) == MSG_GEN_SUCCESS) {
			int *dest_socket = (int *)msg.endpoint;
			must_write(*dest_socket, &msg.type, sizeof(int));
		}
	}
